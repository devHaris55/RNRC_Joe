...
<div class="py-4">
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb breadcrumb-dark breadcrumb-transparent">
            ...
            <li class="breadcrumb-item active" aria-current="page">Rejected-Conflict Request</li>
        </ol>
    </nav>
    <div class="d-flex justify-content-between w-100 flex-wrap">
        <div class="mb-3 mb-lg-0">
            <h1 class="h4">Rejected-Conflict Request-List</h1>
        </div>
        <!-- <div>
                    ...
                    <!-- Item -->
                        ...
                        @foreach($assigned as $key=>$assign)
                            <tr>
                                ...
                                <td class="border-0 font-weight-bold">{{ $assign->first_name.' '.$assign->last_name }}</td>

                                <td class="border-0 font-weight-bold">{{  $assign->email }}</td>
                                <td class="border-0 font-weight-bold">@if($assign->form_type==1)Partial Day @elseif($assign->form_type==2) Full Day @else Reccuring Day @endif</td>
                                <td class="border-0 font-weight-bold">{{($assign->getevent == '')?'Not Assigned':$assign->getevent->title}}</td>
                                <!-- <td class="border-0 font-weight-bold">{{ $assign->progress_status }}</td> -->

                                <td class="border-0">
                                    <a href="{{route('admin_assigned_edit').'/'.$assign->id}}" class="text-secondary mr-3"><i class="fas fa-edit"></i>Edit</a>
